...
echo 'UserParameter=custom_bash[*], /etc/zabbix/bash_script.sh $1 ' >> /etc/zabbix/zabbix_agentd.conf
...
then
    ...
    echo '    print("Gorbunov Vladimir")' >> /etc/zabbix/test_python_script.py
    echo 'elif (sys.argv[1] == "2"):' >> /etc/zabbix/test_python_script.py
    echo '    result=os.popen("date -I").read()' >> /etc/zabbix/test_python_script.py
    echo '    print(result)' >> /etc/zabbix/test_python_script.py
...
if [[ ! -f /etc/zabbix/bash_script.sh ]]
then
    echo '#!/bin/bash' >> /etc/zabbix/bash_script.sh
    echo 'case $1 in' >> /etc/zabbix/bash_script.sh
    echo '    "1")' >> /etc/zabbix/bash_script.sh
    echo '        echo "Gorbunov Vladimir"' >> /etc/zabbix/bash_script.sh
    echo '    ;;' >> /etc/zabbix/bash_script.sh
    echo '    "2")' >> /etc/zabbix/bash_script.sh
    echo '        date -I' >> /etc/zabbix/bash_script.sh	
    echo '    ;;' >> /etc/zabbix/bash_script.sh
    echo '    *)' >> /etc/zabbix/bash_script.sh
    echo '        echo "unknown input"' >> /etc/zabbix/bash_script.sh	
    echo '    ;;' >> /etc/zabbix/bash_script.sh
    echo 'esac' >> /etc/zabbix/bash_script.sh
	chmod +x /etc/zabbix/bash_script.sh
fi
